import {entityProfileFields} from './entity-fields.mjs';
...
export function auditVisibleFactContent(source, ir, { renderedModules = null } = {}) {
  ...
  for (const slide of ir.slides || []) for (const module of slide.modules || []) {
    ...
    for (const fact of module.visibleFacts || []) {
      ...
      else if(units.get(id).kind==='image' && rendered?.verifiedSourceImages?.includes(id)) {
        if(!mapped.has(id)) mapped.set(id,[]);
        mapped.get(id).push({slideId:slide.id,moduleId:module.id,text:'',verifiedSourceImage:true});
      }
  ...
  for (const id of required) {
    ...
    if(unit.kind==='image'&&destinations.every(d=>d.verifiedSourceImage)) continue;
