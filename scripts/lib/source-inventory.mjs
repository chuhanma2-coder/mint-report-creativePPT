...
export async function docxInventory(file) {
  ...
  const unsupported=names.filter(name=>/^word\/(charts\/chart\d+\.xml|embeddings\/)/i.test(name)&&!zip.files[name].dir);
  for(const name of names.filter(name=>/^word\/(document|footnotes|endnotes|header\d+|footer\d+)\.xml$/i.test(name))) {
    const xml=await zip.file(name).async('string');
    if(/<w:(?:object|altChunk)\b|<c:chart\b/i.test(xml)) unsupported.push(name);
  }
  for(const name of names.filter(name=>/^word\/_rels\/.*\.rels$/i.test(name))) {
    const xml=await zip.file(name).async('string');
    if(/relationships\/(?:chart|package|oleObject)/i.test(xml)) unsupported.push(name);
  }
  if(unsupported.length) throw new Error(`SOURCE_PART_REVIEW_REQUIRED: ${[...new Set(unsupported)].join(', ')}; export and verify embedded charts/objects before PPT authoring`);
