...
- 默认导入 `Mint_PPT_16比9-template.pptx`。模板统一母版、字体、品牌色、线条语言和自动页码，但不固定内容槽位；模板没有的图形由 Agent 使用同一视觉语言组合原生对象。
...
任务卡只统一：章节顺序、负责人、来源范围、汇报受众/目的、Skill与母版版本。它不规定页数和版式。真正保证合并后格式基本一致的是：**相同 Skill 版本 + 相同原生母版版本 + 相同品牌合同**。
